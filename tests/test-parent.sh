...
${OSTREE} --repo=repo2 remote add --gpg-import=${test_tmpdir}/gpghome/trusted/pubring.gpg --set=gpg-verify=true aremote file://$(pwd)/repo test2

# Create a repo with repo2 as parent
${OSTREE} init --repo=repo3 --mode=bare-user
${OSTREE} config --repo=repo3 set core.parent `pwd`/repo2

# Ensure the unsigned pull fails so we know we imported the gpg config correctly
if ${OSTREE} --repo=repo3 pull aremote; then
...
${OSTREE} --repo=repo3 pull aremote
