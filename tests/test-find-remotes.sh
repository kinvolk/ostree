...
ostree_repo_init apps-collection --collection-id apps-collection
mkdir -p files
pushd files
...
mkdir os-collection
ostree_repo_init os-collection --collection-id os-collection
mkdir -p files
pushd files
...
${CMD_PREFIX} ostree --repo=local remote add apps-remote file://$(pwd)/apps-collection --collection-id apps-collection --gpg-import=${test_tmpdir}/gpghome/key1.asc
${CMD_PREFIX} ostree --repo=local remote add os-remote file://$(pwd)/os-collection --collection-id os-collection --gpg-import=${test_tmpdir}/gpghome/key2.asc
...
${CMD_PREFIX} ostree --repo=local-mirror remote add apps-remote file://$(pwd)/apps-collection --collection-id apps-collection --gpg-import=${test_tmpdir}/gpghome/key1.asc
${CMD_PREFIX} ostree --repo=local-mirror remote add os-remote file://$(pwd)/os-collection --collection-id os-collection --gpg-import=${test_tmpdir}/gpghome/key2.asc
...
assert_file_has_content refs "^(apps-collection, app1)$"
assert_file_has_content refs "^(os-collection, os/amd64/master)$"

for repo in local local-mirror; do
    # Try finding an update for an existing branch.
    ${CMD_PREFIX} ostree --repo=$repo find-remotes apps-collection app1 > find
    ...
    assert_file_has_content find "^    - (apps-collection, app1) = $(cat app1-checksum)$"
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes apps-collection app1 os-collection os/amd64/master > find
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/apps-collection$"
    assert_file_has_content find "^ - Keyring: apps-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (apps-collection, app1) = $(cat app1-checksum)$"
    ...
    assert_file_has_content find "^    - (os-collection, os/amd64/master) = $(cat os-checksum)$"
    assert_file_has_content find "^2/2 refs were found.$"
    assert_not_file_has_content find "^No results.$"

    # Find some updates and a new branch.
    ${CMD_PREFIX} ostree --repo=$repo find-remotes apps-collection app1 apps-collection app2 os-collection os/amd64/master > find
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/apps-collection$"
    assert_file_has_content find "^ - Keyring: apps-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (apps-collection, app1) = $(cat app1-checksum)$"
    assert_file_has_content find "^    - (apps-collection, app2) = $(cat app2-checksum)$"
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/os-collection$"
    assert_file_has_content find "^ - Keyring: os-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (os-collection, os/amd64/master) = $(cat os-checksum)$"
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes apps-collection app1 apps-collection not-an-app > find
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/apps-collection$"
    assert_file_has_content find "^ - Keyring: apps-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (apps-collection, not-an-app) = (not found)$"
    assert_file_has_content find "^    - (apps-collection, app1) = $(cat app1-checksum)$"
    assert_file_has_content find "^Refs not found in any remote:$"
    assert_file_has_content find "^ - (apps-collection, not-an-app)$"
    assert_file_has_content find "^1/2 refs were found.$"
    assert_not_file_has_content find "^No results.$"

    # Do all the above, but pull this time.
    ${CMD_PREFIX} ostree --repo=$repo find-remotes --pull apps-collection app1 > pull || true
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes --pull apps-collection app1 os-collection os/amd64/master > pull
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes --pull apps-collection app1 apps-collection app2 os-collection os/amd64/master > pull
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes --pull apps-collection app1 apps-collection not-an-app > pull
...
for repo in local-mirror; do
    ...
    ${CMD_PREFIX} ostree --repo=$repo find-remotes os-collection os/amd64/master > find
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/os-collection$"
    assert_file_has_content find "^ - Keyring: os-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (os-collection, os/amd64/master) = $(cat os-checksum-2)$"
    assert_file_has_content find "^1/1 refs were found.$"
    assert_not_file_has_content find "^No results.$"

    # Pull it.
    ${CMD_PREFIX} ostree --repo=$repo find-remotes --pull os-collection os/amd64/master > pull || true
    assert_file_has_content pull "^1/1 refs were found.$"
    assert_file_has_content pull "^Pulled 1/1 refs successfully.$"
    assert_not_file_has_content pull "Failed to pull some refs from the remotes"
...
${CMD_PREFIX} ostree --repo=local remote add os-remote-local-mirror file://$(pwd)/local-mirror --collection-id os-collection --gpg-import=${test_tmpdir}/gpghome/key2.asc
...
for repo in local; do
    ...
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/os-collection$"
    assert_file_has_content find "^ - Keyring: os-remote.trustedkeys.gpg$"
    assert_file_has_content find "^    - (os-collection, os/amd64/master) = $(cat os-checksum-3)$"
    assert_file_has_content find "^Result [0-9]\+: file://$(pwd)/local-mirror$"
    assert_file_has_content find "^ - Keyring: os-remote-local-mirror.trustedkeys.gpg$"
    assert_file_has_content find "^    - (os-collection, os/amd64/master) = $(cat os-checksum-3)$"
    assert_file_has_content find "^1/1 refs were found.$"
